...
import gdb2dict.gdb_value_to_dict as gdb_to_dict
...
# Set a breakpoint
gdb.Breakpoint("inspectContainerObject", temporary=True)
...
try:
    ...
    container_obj_ptr = frame.read_var("obj")

    if container_obj_ptr is None:
        print("Error: Could not read variable 'obj'.")
        gdb.execute("quit 1")

    # Convert the gdb.Value to a dictionary
    result_dict = gdb_to_dict.gdb_value_to_dict(container_obj_ptr)
