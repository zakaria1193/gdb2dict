...
run_test() {
    ...
    if gdb -batch \
        ...
        sed -n '/^{/,$p' "${actual_output_file}" > "${actual_output_file}.tmp"
        mv "${actual_output_file}.tmp" "${actual_output_file}"
        sed -i -E 's/"reference_to_contained": "@0x[0-9a-f]+"/"reference_to_contained": "@0xPTR"/' "${actual_output_file}"
        sed -i -E 's/"reference_to_contained": "@0x[0-9a-f]+"/"reference_to_contained": "@0xPTR"/' "${expected_output_file}"
        sed -i -E 's/("-0x[0-9a-f]+"|"0x[0-9a-f]+")/"0x0"/g' "${actual_output_file}"
        sed -i -E 's/("-0x[0-9a-f]+"|"0x[0-9a-f]+")/"0x0"/g' "${expected_output_file}"
